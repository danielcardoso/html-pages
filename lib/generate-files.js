...
_.each(jsonObject, (item, key) => {
  ...
  const filePath = path.normalize(path.join(__dirname, '/../public/images/icons/' + iconName + '.png'));
    ...
    _.each(item.aliases, (alias) => {
      if (_.isArray(alias.extensions)) {
        _.each(alias.extensions, (extension, key) => {
          const fileName = (extension.indexOf('.') === 0 ? '' : alias.name + '.') + extension;
...
const message = chalk.blue(`There are:\n`) +
  chalk.green(`    ${chalk.bold(`${_.size(statistics.withIcon)}`)} groups of extensions with icon!\n`) +
  chalk.red(`    ${chalk.bold(`${_.size(statistics.withoutIcon)}`)} groups of extensions without icon!`)
