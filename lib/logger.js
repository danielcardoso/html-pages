...
module.exports = (defaultLevel) => {
  ...
  self.setLevel = (level) => {
    if (_.isString(level) && _.isNumber(self._levels[level.toUpperCase()])) {
      ...
      console.log(color.yellow('Warning!') + ' The log-level "' + level +
        '" is unknown. The default option will be used.');
    }
  };
  ...
  self.logRequest = (code, method, url, userAgent = '') => {
    ...
    message += color.italic.gray(' - [' + (global.utils.getMsTime() - global.requestTime).toFixed(2) + 'ms]');
