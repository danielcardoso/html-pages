...
module.exports = () => {
  ...
  _.each(options, (item, key) => {
    ...
    if (_.isObject(opt) && _.isString(opt.validation)) {
      ...
      // console.log('KEY: ' + key);
      // console.log('Obj', opt);
      // console.log('isObject', _.isObject(opt));
      // console.log('isString', _.isString(opt.validation), opt.validation);
      // console.log('isFunction', _.isFunction(validator[opt.validation]));
      //
      // process.exit(1);
    }
