...
const parseListItem = function(item, getIconFromFileName) {
  ...
  if (item.type === 'parent') {
    ...
    item.iconPath = 'icons/' + getIconFromFileName(item.base, item.ext) + '.png';
...
module.exports = coroutine(function*(port, current, dir, ignorePattern, getIconFromFileName) {
  let filesRaw = [];
  ...
  for (const file of filesRaw) {
    ...
    if (ignorePattern.isValidPath(details.relative)) {
      ...
      details.stats.date = moment(fileStats.mtime).format('DD-MMM-YYYY HH:mm');
      details.stats.birthtime = moment(fileStats.birthtime).format('DD-MMM-YYYY HH:mm');
  ...
  const details = {
    ...
    author: {
      username: pkg.author.github.username,
      link: pkg.author.github.link
    },
    project: {
      parent: hasParent,
      files,
