...
module.exports = coroutine(function*(req, res, flags, current, ignorePattern) {
  const headers = {};
  ...
  if (flags.auth) {
    ...
    if (!process.env.PAGES_USER || !process.env.PAGES_PASSWORD) {
      console.error(
        red(
          'You are running `' + pkg.name + '` with basic auth but did not set the ' +
  ...
  try {
    const view404Path = path.normalize(path.join(__dirname, '/../views/error-404.hbs'));
  ...
  if (pathType.dirSync(related)) {
    ...
    if (flags.directoryIndex !== false && fs.existsSync(directoryIndexPath)) {
      res.setHeader('Content-Type', mime.contentType(path.extname(directoryIndexPath)));
